    ...
    for b in range(n):
        ...
        if 小组[0]['Target Name']==内参基因:
            ...
            for c in range(k):
                ...
                if 单孔['Omit']==False:
                    '''这组加样孔是有效的'''
                    ΔCt_SD=(单孔['Ct SD']**2+take_data_safely(data_dict_list,a*n*k,'Ct SD')**2)**0.5
...
#添加ΔΔCt、ΔΔCt的平均值、ΔΔCt的样本标准差、2^-ΔΔCt、2^ΔΔCt的平均值和2^ΔΔCt的样本标准差
keys=keys+('ΔΔCt','ΔΔCt Mean','ΔΔCt SD','2^(-ΔΔCt)','2^(-ΔΔCt) Mean','2^(-ΔΔCt) SD')
for a in range(m):
    大组=data_dict_list[(a*n*k):((a+1)*n*k)]
    if 大组[0]['Sample Name']==原始株:
        ...
        for b in range(n*k):
            单孔=大组[b]
            单孔.update({'ΔΔCt':'','ΔΔCt Mean':'','ΔΔCt SD':'','2^(-ΔΔCt)':'','2^(-ΔΔCt) Mean':'','2^(-ΔΔCt) SD':''})
    else:
        '''这不是原始株'''
        for b in range(n):
            小组=大组[(b*k):((b+1)*k)]
            if 小组[0]['Target Name']==内参基因:
                ...
                for c in range(k):
                    ...
                    if 单孔['Omit']==False:
                        '''这组加样孔是有效的'''
                        ΔΔCt_SD=(单孔['ΔCt SD']**2+take_data_safely(data_dict_list,b*k,'ΔCt SD')**2)**0.5
                        '''方差的传递'''
                        单孔.update({'ΔΔCt SD':ΔΔCt_SD})
                    else:
                        '''这组加样孔是无效的'''
                        单孔.update({'ΔΔCt SD':''})
                #然后加2^-ΔΔCt
                ...
                for c in range(k):
                    ...
                    if 单孔['Omit']==False:
                        '''这组加样孔是有效的'''
                        单孔.update({'2^(-ΔΔCt) Mean':指数化_Mean})
                    else:
                        '''这组加样孔是无效的'''
                        单孔.update({'2^(-ΔΔCt) Mean':''})
                #然后计算并添加2^-ΔΔCt的样本标准差
                for c in range(k):
                    单孔=小组[c]
                    if 单孔['Omit']==False:
                        '''这组加样孔是有效的'''
                        指数化_SD=单孔['ΔΔCt SD']*(2**-(单孔['ΔΔCt Mean']))*math.log(2)
                        '''方差经函数变化后的值'''
                        单孔.update({'2^(-ΔΔCt) SD':指数化_SD})
                    else:
                        '''这组加样孔是无效的'''
                        单孔.update({'2^(-ΔΔCt) SD':''})
